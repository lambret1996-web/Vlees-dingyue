...
export async function onRequest(request){
    ...
    if(!uuid){
        return new Response("UUID missing", {status:400});
    }
    if(!validUUID(uuid)){
...
function generateVless(config){
    ...
    for(const node of config.nodes){
        ...
        const link = `vless://${config.uuid}@${node.host}:${config.port}?${params.toString()}#${encodeURIComponent(remark)}`;
        list.push(link);
    }
...
<title>VLESS Global Generator PRO</title>
<style>
...
label{display:block;margin-top:18px;font-size:13px;color:#cbd5e1;}
input,select{
width:100%;margin-top:8px;padding:14px;
border-radius:14px;border:1px solid rgba(255,255,255,.2);
background:rgba(0,0,0,.25);color:white;font-size:15px;
}
button{
width:100%;margin-top:20px;padding:15px;border:0;border-radius:16px;
...
background:rgba(0,0,0,.25);word-break:break-all;font-size:13px;
}
.qrcode{margin-top:20px;text-align:center;}
.qrcode img{width:200px;background:white;padding:10px;border-radius:15px;}
.info{margin-top:15px;font-size:13px;color:#94a3b8;text-align:center;}
</style>
</head>
<body>
<div class="card">
<h1>🚀 VLESS Global Generator</h1>
<label>UUID</label>
...
<option value="v2ray">VLESS Base64</option>
...
<div class="info">全球节点数量:<span id="nodes">5</span></div>
</div>
<script>
const KEY="vless_uuid";
const uuidInput=document.getElementById("uuid");
uuidInput.value = localStorage.getItem(KEY)||"";
let current="";
function generate(){
...
current = location.origin+"/?uuid="+encodeURIComponent(uuid)+"&port="+port;
if(format!=="auto") current += "&format="+format;
document.getElementById("result").innerHTML = \`<a href="\${current}" target="_blank">\${current}</a>\`;
document.getElementById("qr").src="https://api.qrserver.com/v1/create-qr-code/?size=220x220&data="+encodeURIComponent(current);
}
function copyUrl(){
if(!current){alert("请先生成");return;}
